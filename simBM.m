...
for b = 1:length(unique(data.block))
    ...
    for trial = 1:sum(data.block==b) %ntrials
        ...
        % learn about p_r0) and p(r1) 
        % p(R=1|correct) = the probability of getting a 1 when correct
        % p(R=1|incorrect) = the probability of getting a 1 when incorrect
...
%         [~,which] = max(p_r0); r0 = xs(which);
        ...
        r1 = sum(xs.*p_r1); r0 = sum(xs.*p_r0); %integrate over p_r1, p_r0
        
        if sum(isnan(p_r1))>0 | sum(isnan(p_r0))>0 %if there's a NaN present, throw a flag
            broken = true
        end
